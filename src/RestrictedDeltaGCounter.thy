theory
  RestrictedDeltaGCounter
imports
  Network
begin

type_synonym ('id) state = "'id \<Rightarrow> int option"
type_synonym ('id) operation = "'id \<times> int"
...
fun op_to_state :: "('id operation) \<Rightarrow> ('id state)" where
"op_to_state (who, count) = (\<lambda>x. if x = who then Some count else None)"

fun delta_gcounter_op :: "('id operation) \<Rightarrow> ('id state) \<rightharpoonup> ('id state)" where
"delta_gcounter_op theirs ours = Some (\<lambda> x. option_max ((op_to_state theirs) x) (ours x))"
