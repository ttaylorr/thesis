...
\begin{document}
  ...
  \end{frame}
    ...
    % \begin{description}
    %   \item[State-based] The \textit{state-based} CRDT gossips its entire state
    %     as a payload to other CRDTs.
    %   \item[Op-based] The \textit{op-based} CRDT sends an \textit{effector}
    %     (representing a possibly-mutable operation) to be executed synchronously
    %     at each downstream replica.
    %   \item[$\delta$-based] The \dcrdt gossips the state
    % \end{description}
  \begin{frame}
    \printbibliography
  \end{frame}
