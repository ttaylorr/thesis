...
datatype ('proc, 'msg) send
  = Send (msg_recipients: \<open>'proc set\<close>) (send_msg: 'msg)

fun unicast :: \<open>'proc \<Rightarrow> 'msg \<Rightarrow> ('proc, 'msg) send \<close> where
  \<open>unicast proc msg = (Send {proc} msg)\<close>
...
fun valid_event :: \<open>('proc, 'msg, 'val) event \<Rightarrow> 'proc \<Rightarrow>
  ...
  \<open>valid_event (Receive sender msg) proc msgs =
    (\<exists> procs. proc \<in> procs \<and> (sender, Send procs msg) \<in> msgs)\<close> |
...
lemma execute_receive:
  ...
  shows \<open>\<exists> recpts. recpt \<in> recpts \<and> (sender, Send recpts msg) \<in> msgs'\<close>
  using assms execute_step by auto
end