...
fun acceptor_step where
  \<open>acceptor_step state (Receive sender (Propose val)) =
     (case state of
        None   \<Rightarrow> (Some val, {unicast sender (Accept val)}) |
        Some v \<Rightarrow> (Some v,   {unicast sender (Accept v)}))\<close> |
  \<open>acceptor_step state _ = (state, {})\<close>

fun proposer_step where
  \<open>proposer_step None (Request val) = (None, {unicast 0 (Propose val)})\<close> |
  \<open>proposer_step None (Receive _ (Accept val)) = (Some val, {})\<close> |
  \<open>proposer_step state _ = (state, {})\<close>

fun consensus_step ::  "'a set
     \<Rightarrow> 'b option
        \<Rightarrow> ('c, 'b msg, 'b) event
           \<Rightarrow> 'b option \<times> ('c, 'b msg) send set"  where
  \<open>consensus_step procs =
     (if 0 \<in> procs then acceptor_step else proposer_step)\<close>
...
definition inv1 where
  \<open>inv1 msgs states \<longleftrightarrow>
     (\<forall>proc val. proc \<noteq> 0 \<and> states proc = Some val \<longrightarrow>
                 (\<exists>sender. (sender, unicast proc (Accept val)) \<in> msgs))\<close>
...
lemma invariant_propose:
  assumes \<open>msgs' = msgs \<union> {(proc, unicast 0 (Propose val))}\<close>
...
proof -
  ...
  then show ?thesis
  by (metis (full_types) assms(2) inv1_def inv2_def unicast.elims)
qed
...
proof -
  {
    ...
    hence \<open>\<exists>sender. (sender, unicast p (Accept v)) \<in> msgs\<close>
...
lemma invariant_learn:
  assumes \<open>states' = states (proc := Some val)\<close>
    and \<open>(sender, unicast proc (Accept val)) \<in> msgs\<close>
    and \<open>inv1 msgs states \<and> inv2 msgs states\<close>
  shows \<open>inv1 msgs states' \<and> inv2 msgs states'\<close>
proof -
  {
    fix p v
    assume asm: \<open>p \<noteq> 0 \<and> states' p = Some v\<close>
    have \<open>\<exists>sender. (sender, unicast p (Accept v)) \<in> msgs\<close>
  ...
  moreover {
    ...
    hence \<open>states' 0 = Some v\<close>
      by (metis (no_types, lifting) assms(1) assms(2) assms(3) inv2_def map_upd_Some_unfold unicast.elims)
  }
  ultimately show ?thesis
    by (simp add: inv1_def inv2_def)
qed
